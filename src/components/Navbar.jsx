    ...
    return (
        <>
            <nav className=' w-full   mx-auto top-0 left-0 '>
                ...
                {/* Navigation Bar */}
                <ul className={`md:flex md:items-center bg-gray-900 z-[1] select-none  md:my-0 md:z-auto duration-500 ease-in absolute
             text-white flex-row md:items-center md:text-sm w-full ${menu ? 'left-0' : 'hidden'}`}>
                 ...
                 font-robot text-[#F86615]`}>{restaurantInfos[0].name}</li>
                    ...
                    {/* Dropdown Language */}
                    <li className='ltr:ml-10 rtl:mr-10 right-0 ltr:float-right block rtl:float-left'>
                        <DropLanguage/>
                    </li>
                    ...
                    {/* Cart Shooping Button */}
                     <li className='cursor-pointer md:visible invisible p-6 ltr:float-right rtl:pl-12 mx-10'>
            ...
            {/* Side drawer Orders */}
            <div className={nav ? 'fixed z-[20] top-0 shadow-2xl md:visible invisible ltr:right-0 rtl:left-0 w-[300px] h-screen bg-white z-10 duration-300' : 'fixed top-0  right-[-100%] w-[300px] h-screen bg-white z-10 duration-300'}>
                <AiOutlineClose
                    onClick={() => setNav(!nav)}
                    size={30}
